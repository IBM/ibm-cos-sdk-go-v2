...
func PutBucketReplication() {
	...
	fmt.Println("Setting replication configuration for bucket:", bucket)

	out, err := cosClient.PutBucketReplication(context.TODO(), &s3.PutBucketReplicationInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("PutBucketReplication error:", err)
		return
	}
