...
func GetBucketVersioning() {
	...
	fmt.Println("Getting versioning configuration for bucket:", bucket)

	out, err := cosClient.GetBucketVersioning(context.TODO(), &s3.GetBucketVersioningInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("GetBucketVersioning error:", err)
		return
	}
