...
func DeleteLegalHold() {
	...
	fmt.Println("Deleting legal hold for bucket:", bucket)

	out, err := cosClient.DeleteLegalHold(context.TODO(), &s3.DeleteLegalHoldInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("DeleteLegalHold error:", err)
		return
	}
