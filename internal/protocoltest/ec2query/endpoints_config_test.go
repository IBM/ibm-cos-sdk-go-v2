...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-ec2-protocol.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-ec2-protocol.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-ec2-protocol.dev",
			expectURL:     aws.String("https://env-ec2-protocol.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-ec2-protocol.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-ec2-protocol.dev",
			expectURL:     aws.String("http://config-ec2-protocol.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-ec2-protocol.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-ec2-protocol.dev",
			clientEndpoint: aws.String("https://client-ec2-protocol.dev"),
			expectURL:      aws.String("https://client-ec2-protocol.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_EC2_PROTOCOL", c.envService)
			}
