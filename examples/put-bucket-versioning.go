...
func PutBucketVersioning() {
	...
	fmt.Println("Setting versioning configuration for bucket:", bucket)

	out, err := cosClient.PutBucketVersioning(context.TODO(), &s3.PutBucketVersioningInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("PutBucketVersioning error:", err)
		return
	}
