...
func ListParts() {
	...
	fmt.Println("Listing parts of multipart upload for bucket:", bucket)

	out, err := cosClient.ListParts(context.TODO(), &s3.ListPartsInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("ListParts error:", err)
		return
	}
