...
func GetBucketTagging() {
	...
	fmt.Println("Getting bucket tagging for bucket:", bucket)

	out, err := cosClient.GetBucketTagging(context.TODO(), &s3.GetBucketTaggingInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("GetBucketTagging error:", err)
		return
	}
