...
func DeleteObjectTagging() {
	...
	fmt.Println("Deleting object tagging for bucket:", bucket)

	out, err := cosClient.DeleteObjectTagging(context.TODO(), &s3.DeleteObjectTaggingInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("DeleteObjectTagging error:", err)
		return
	}
