...
func ListMultipartUploads() {
	...
	fmt.Println("Listing multipart uploads for bucket:", bucket)

	out, err := cosClient.ListMultipartUploads(context.TODO(), &s3.ListMultipartUploadsInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("ListMultipartUploads error:", err)
		return
	}
