...
func GetObjectLockConfiguration() {
	...
	fmt.Println("Getting object lock configuration for bucket:", bucket)

	out, err := cosClient.GetObjectLockConfiguration(context.TODO(), &s3.GetObjectLockConfigurationInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("GetObjectLockConfiguration error:", err)
		return
	}
