...
func GetObjectRetention() {
	...
	fmt.Println("Getting object retention for bucket:", bucket)

	out, err := cosClient.GetObjectRetention(context.TODO(), &s3.GetObjectRetentionInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("GetObjectRetention error:", err)
		return
	}
