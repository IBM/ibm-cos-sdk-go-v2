...
func PutBucketCors() {
	...
	fmt.Println("Setting CORS configuration for bucket:", bucket)

	out, err := cosClient.PutBucketCors(context.TODO(), &s3.PutBucketCorsInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("PutBucketCors error:", err)
		return
	}
