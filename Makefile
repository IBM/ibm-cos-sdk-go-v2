...
copy-attributevalue-feature:
	...
	ls *.go | grep -v "convert.go" | grep -v "doc.go" | grep -v "go_module_metadata.go" | \
		xargs -I % sed -i.bk -E 's:github.com/IBM/ibm-cos-sdk-go-v2/(service|feature)/dynamodb:github.com/IBM/ibm-cos-sdk-go-v2/\1/dynamodbstreams:g' % &&  \
	ls *.go | grep -v "convert.go" | grep -v "doc.go" | grep -v "go_module_metadata.go" | \
...
external-changelog:
	...
	@echo "See CONTRIBUTING.md 'Changelog Documents' and an example at https://github.com/IBM/ibm-cos-sdk-go-v2/pull/2934/files"
...
list-deps-%:
	...
	@cd ./internal/repotools/cmd/eachmodule \
		...
		"go list -m all | grep -v 'github.com/IBM/ibm-cos-sdk-go-v2'" | sort -u
