...
func PutBucketTagging() {
	...
	fmt.Println("Setting bucket tagging for bucket:", bucket)

	out, err := cosClient.PutBucketTagging(context.TODO(), &s3.PutBucketTaggingInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("PutBucketTagging error:", err)
		return
	}
