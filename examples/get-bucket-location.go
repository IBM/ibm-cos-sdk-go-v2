...
func GetBucketLocation() {
	...
	fmt.Println("Getting bucket location for bucket:", bucket)

	out, err := cosClient.GetBucketLocation(context.TODO(), &s3.GetBucketLocationInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("GetBucketLocation error:", err)
		return
	}
