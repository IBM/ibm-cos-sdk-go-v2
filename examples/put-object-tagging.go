...
func PutObjectTagging() {
	...
	fmt.Println("Setting object tagging for bucket:", bucket)

	out, err := cosClient.PutObjectTagging(context.TODO(), &s3.PutObjectTaggingInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("PutObjectTagging error:", err)
		return
	}
