...
func ListLegalHolds() {
	...
	fmt.Println("Listing legal holds for bucket:", bucket)

	out, err := cosClient.ListLegalHolds(context.TODO(), &s3.ListLegalHoldsInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("ListLegalHolds error:", err)
		return
	}
