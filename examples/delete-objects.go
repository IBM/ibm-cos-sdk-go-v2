...
func DeleteObjects() {
	...
	fmt.Println("Deleting multiple objects for bucket:", bucket)

	out, err := cosClient.DeleteObjects(context.TODO(), &s3.DeleteObjectsInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("DeleteObjects error:", err)
		return
	}
