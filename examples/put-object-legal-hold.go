...
func PutObjectLegalHold() {
	...
	fmt.Println("Setting object legal hold for bucket:", bucket)

	out, err := cosClient.PutObjectLegalHold(context.TODO(), &s3.PutObjectLegalHoldInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("PutObjectLegalHold error:", err)
		return
	}
