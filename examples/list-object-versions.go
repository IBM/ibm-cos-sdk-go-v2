...
func ListObjectVersions() {
	...
	fmt.Println("Listing object versions for bucket:", bucket)

	out, err := cosClient.ListObjectVersions(context.TODO(), &s3.ListObjectVersionsInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("ListObjectVersions error:", err)
		return
	}
