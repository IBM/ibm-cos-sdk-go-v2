...
func DeleteBucketReplication() {
	...
	fmt.Println("Deleting replication configuration for bucket:", bucket)

	out, err := cosClient.DeleteBucketReplication(context.TODO(), &s3.DeleteBucketReplicationInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("DeleteBucketReplication error:", err)
		return
	}

	fmt.Println("Replication configuration deleted successfully")
	fmt.Println(out)
}
