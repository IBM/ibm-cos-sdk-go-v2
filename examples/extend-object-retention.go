...
func ExtendObjectRetention() {
	...
	fmt.Println("Extending object retention for bucket:", bucket)

	out, err := cosClient.ExtendObjectRetention(context.TODO(), &s3.ExtendObjectRetentionInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("ExtendObjectRetention error:", err)
		return
	}
