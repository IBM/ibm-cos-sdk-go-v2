...
func DeleteBucketTagging() {
	...
	fmt.Println("Deleting tagging configuration for bucket:", bucket)

	out, err := cosClient.DeleteBucketTagging(context.TODO(), &s3.DeleteBucketTaggingInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("DeleteBucketTagging error:", err)
		return
	}
