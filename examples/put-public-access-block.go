...
func PutPublicAccessBlock() {
	...
	fmt.Println("Setting public access block configuration for bucket:", bucket)

	out, err := cosClient.PutPublicAccessBlock(context.TODO(), &s3.PutPublicAccessBlockInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("PutPublicAccessBlock error:", err)
		return
	}
