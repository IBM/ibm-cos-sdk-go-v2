...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-s3.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-s3.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-s3.dev",
			expectURL:     aws.String("https://env-s3.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-s3.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-s3.dev",
			expectURL:     aws.String("http://config-s3.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-s3.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-s3.dev",
			clientEndpoint: aws.String("https://client-s3.dev"),
			expectURL:      aws.String("https://client-s3.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_S3", c.envService)
			}
