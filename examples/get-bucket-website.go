...
func GetBucketWebsite() {
	...
	fmt.Println("Getting website configuration for bucket:", bucket)

	out, err := cosClient.GetBucketWebsite(context.TODO(), &s3.GetBucketWebsiteInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("GetBucketWebsite error:", err)
		return
	}
