...
func RestoreObject() {
	...
	fmt.Println("Restoring archived object for bucket:", bucket)

	out, err := cosClient.RestoreObject(context.TODO(), &s3.RestoreObjectInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("RestoreObject error:", err)
		return
	}
