...
func PutObjectLockConfiguration() {
	...
	fmt.Println("Setting object lock configuration for bucket:", bucket)

	out, err := cosClient.PutObjectLockConfiguration(context.TODO(), &s3.PutObjectLockConfigurationInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("PutObjectLockConfiguration error:", err)
		return
	}
