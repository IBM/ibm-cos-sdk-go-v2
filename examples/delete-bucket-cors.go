...
func DeleteBucketCors() {
	...
	fmt.Println("Deleting CORS configuration for bucket:", bucket)

	out, err := cosClient.DeleteBucketCors(context.TODO(), &s3.DeleteBucketCorsInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("DeleteBucketCors error:", err)
		return
	}
