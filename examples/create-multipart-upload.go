...
func CreateMultipartUpload() {
	...
	fmt.Printf("Creating multipart upload: s3://%s/%s\n", bucket, key)

	out, err := cosClient.CreateMultipartUpload(context.TODO(), &s3.CreateMultipartUploadInput{
	...
	fmt.Println("Multipart upload created successfully")
	fmt.Printf("Upload ID: %s\n", aws.ToString(out.UploadId))
	fmt.Println(out)
}
