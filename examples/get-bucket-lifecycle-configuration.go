...
func GetBucketLifecycleConfiguration() {
	...
	fmt.Println("Getting lifecycle configuration for bucket:", bucket)

	out, err := cosClient.GetBucketLifecycleConfiguration(context.TODO(), &s3.GetBucketLifecycleConfigurationInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("GetBucketLifecycleConfiguration error:", err)
		return
	}
