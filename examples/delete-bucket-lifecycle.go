...
func DeleteBucketLifecycle() {
	...
	fmt.Println("Deleting lifecycle configuration for bucket:", bucket)

	out, err := cosClient.DeleteBucketLifecycle(context.TODO(), &s3.DeleteBucketLifecycleInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("DeleteBucketLifecycle error:", err)
		return
	}

	fmt.Println("Lifecycle configuration deleted successfully")
	fmt.Println(out)
}
