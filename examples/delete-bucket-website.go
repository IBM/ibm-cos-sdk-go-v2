...
func DeleteBucketWebsite() {
	...
	fmt.Println("Deleting website configuration for bucket:", bucket)

	out, err := cosClient.DeleteBucketWebsite(context.TODO(), &s3.DeleteBucketWebsiteInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("DeleteBucketWebsite error:", err)
		return
	}
