...
func GetBucketCors() {
	...
	fmt.Println("Getting CORS configuration for bucket:", bucket)

	out, err := cosClient.GetBucketCors(context.TODO(), &s3.GetBucketCorsInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("GetBucketCors error:", err)
		return
	}
