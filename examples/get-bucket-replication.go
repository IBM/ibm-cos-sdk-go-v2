...
func GetBucketReplication() {
	...
	fmt.Println("Getting replication configuration for bucket:", bucket)

	out, err := cosClient.GetBucketReplication(context.TODO(), &s3.GetBucketReplicationInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("GetBucketReplication error:", err)
		return
	}
