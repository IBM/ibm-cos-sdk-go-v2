...
func GetObjectLegalHold() {
	...
	fmt.Println("Getting object legal hold for bucket:", bucket)

	out, err := cosClient.GetObjectLegalHold(context.TODO(), &s3.GetObjectLegalHoldInput{
		Bucket: aws.String(bucket),
	})
	if err != nil {
		fmt.Println("GetObjectLegalHold error:", err)
		return
	}
